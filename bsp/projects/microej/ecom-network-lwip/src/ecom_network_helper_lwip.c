...
void LLECOM_NETWORK_IMPL_isStarted_action(MICROEJ_ASYNC_WORKER_job_t* job) {
	...
	if (param->netifName == NULL) {
		...
		if(pnetif != NULL){
			if (LLECOM_NETWORK_interface_is_started(pnetif) == 1)
...
void LLECOM_NETWORK_IMPL_useStaticDNS_action(MICROEJ_ASYNC_WORKER_job_t* job) {
	...
	if (param->netifName == NULL) {
		...
		if ((param->use == JTRUE) && !ecom_network_is_static) {
			LLECOM_NETWORK_DEBUG_TRACE("WARNING: a statically set DNS address will be overridden by DHCP when joining a network");
		}
		if ((param->use == JFALSE) && ecom_network_is_static) {
			LLECOM_NETWORK_DEBUG_TRACE("WARNING: a dynamically set DNS address will have no effect until enabling the DHCP");
		}
		param->result = 0;
	}
