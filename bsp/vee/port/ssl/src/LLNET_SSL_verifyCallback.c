 ...
 */
static char cert_info_buf_debug[CERT_INFO_BUF_SIZE] = {0};
...
int LLNET_SSL_VERIFY_verifyCallback(void *data, mbedtls_x509_crt *crt, int depth, uint32_t *flags)
{
	...
	mbedtls_x509_crt_info(&cert_info_buf_debug[0], CERT_INFO_BUF_SIZE - 1, "  ", crt); //-1 to keep last byte for null-terminated character
	printf("%s", &cert_info_buf_debug[0]);

	if (0 == *flags)
		printf("No verification issue for this certificate\n");
	else
	{
		mbedtls_x509_crt_verify_info(&cert_info_buf_debug[0], CERT_INFO_BUF_SIZE, "  ! ", *flags);
		printf("%s\n", &cert_info_buf_debug[0]);
	}
#endif
