...
int64_t microej_time_time_to_tick(int64_t time) {
	...
	volatile int64_t time_microseconds  = time * MILLISECONDS_TO_MICROSECONDS;

	// Check for no overflow
	if (time_microseconds >= 0) {
		volatile int64_t ticks64 = (time_microseconds + (MICROSECONDS_PER_TICK-1)) / MICROSECONDS_PER_TICK;
...
int64_t microej_time_tick_to_time(int64_t ticks) {
    ...
    volatile int64_t time_milliseconds = ticks * MILLISECONDS_PER_TICK;
    // Check for no overflow
...
int64_t microej_time_get_current_time(uint8_t is_platform_time) {
	volatile int64_t ticks = (int64_t) xTaskGetTickCount();
	volatile int64_t time = microej_time_tick_to_time(ticks);

	if (is_platform_time == TRUE) {
		return time;
	} else {
...
int64_t microej_time_get_time_nanos() {
	return microej_time_get_current_time(TRUE) * MILLISECONDS_TO_NANOSECONDS;
}
...
void microej_time_set_application_time(int64_t time) {
	volatile int64_t currentTime = (int64_t) microej_time_get_current_time(TRUE);
